...
class CTCartPoleEnv(gym.Env):
    ...
    def __init__(self, config, model=None, use_sindy=False):
        self.model = model
    ...
    def set_sindy_model(self, new_model):
        self.model = new_model
    
    def set_sindy_use(self, use):
        self.use_sindy = use
        logger.warn("Environment changed model parameters: use_sindy = {}".format(self.use_sindy))
    ...
    def fun_cartpole(self, t, z, f_zoh):
        # print("in function", self.use_sindy)
        if self.use_sindy:
            if self.model is None:
                assert "Need to set SINDy model"
            return self.fun_cartpole_sindy(t, z, f_zoh)
        else:
            return self.fun_cartpole_gt(t, z, f_zoh)

    def fun_cartpole_gt(self, t, z, f_zoh):
    ...
    def fun_cartpole_sindy(self, t, z, f_zoh):
        """
        SINDy dynamics computed derivative
        """
        x, th, xd, thd = z
        c = math.cos(th)
        s = math.sin(th)
        # print(x, c, s, xd, thd, f_zoh)
        # _, _, _, xdd, thdd = self.model.predict(np.array([[x, c, s, xd, thd]]), u=np.array([f_zoh]))[0]
        xdd = 13.805 * f_zoh + 1.497 * c * s + 0.160 * c * xd + 0.038 * s * f_zoh + -0.930 * np.sin(1 * x) + 0.960 * np.sin(1 * s) + -11.045 * np.sin(1 * f_zoh)
        thdd = -0.126 * xd + 0.579 * f_zoh + -4.871 * x * f_zoh + -2.513 * c * s + 0.461 * c * xd + -7.965 * c * f_zoh + 0.013 * s * f_zoh + -1.662 * f_zoh**2 + 0.284 * np.sin(1 * c) + -23.299 * np.sin(1 * s)
        dzdt = [xd, thd, xdd, thdd]
        return dzdt
