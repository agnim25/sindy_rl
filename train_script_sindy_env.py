...
method = 'ppo2'
settings = [('rk', 'swingupsafe', 'dm')]
setting_render = ('rk', 'swingupsafe', 'dm')
...
for setting in settings:
    ...
    env.set_sindy_use(True)
    env.set_sindy_model(sindy_model)
    env = CosineSineObservation(env)
    eval_env = gym.make('ct_{}-{}'.format(env_name, ver))
    eval_env.set_sindy_use(True)
    eval_env.set_sindy_model(sindy_model)
